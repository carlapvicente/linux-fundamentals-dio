...
- `gerenciar_pastas.sh`: Módulo centralizado para criar, remover e validar diretórios e grupos.
- `gerenciar_usuarios.sh`: Módulo centralizado para criar, remover e validar usuários.
- `setup.sh`: Script de configuração inicial que verifica dependências (como `openssl`) e concede as permissões de execução necessárias.
...
- `config.conf`: Arquivo de configuração para variáveis globais, como a senha padrão e o nome do arquivo de log.
...
- O interpretador `bash` e o `openssl` instalados.

### Passo a Passo

Na mesma pasta onde estão todos os scripts, execute os passos abaixo:

1.  **Crie o arquivo `usuarios.txt`**

    Crie um arquivo chamado `usuarios.txt`. Cada linha deve seguir o formato `login:Nome Completo:setor`, sem espaços ao redor dos dois-pontos.
    ...
    **Exemplo de `usuarios.txt`:**
    ...
    ```
2.  **(Opcional) Edite o arquivo de configuração**
    Abra o arquivo `config.conf` para alterar a senha padrão (`SENHA_PADRAO`) ou o nome do arquivo de log (`LOG_FILE`).

3. **Dê permissão de execução ao script setup.sh**
    
    ```bash
    chmod +x setup.sh
    ```
1. **Execute o Script de Configuração**
    Execute o script `setup.sh` uma única vez para preparar o ambiente. Ele verificará as dependências e tornará todos os scripts `.sh` executáveis.
    ```bash
    ./setup.sh
    ```
...
4.  **Execute o Script Principal**
    ...
    Você verá o menu principal com as opções:

    ```
    ========================================
         GERENCIADOR DE INFRAESTRUTURA IT
    ========================================
    1 - Criar diretório(s) e grupo(s)
    2 - Criar usuário(s)
    3 - Remover diretório(s) e grupo(s)
    4 - Remover usuário(s)
    5 - Sair do sistema
    ========================================
    ```

    Basta escolher a opção desejada para executar a automação correspondente.

    > Caso seja digitado algo que não seja os números das opções disponíveis, será exibida a seguinte mensagem:
    >```
    > "================================"
    > "❌ OPERAÇÃO INVÁLIDA..."
...
**Explicação do valor de permissão octal `1777`**:

| Octal | Significado | Permissão Visual (ls -l) |
| ----- | ----------- | ---------------- |
| 1 | Sticky Bit (1) | `rwxrwxrwt` (t = Sticky Bit) |
| 7 | Dono (root) | `rwx` |
| 7 | Grupo (GRP_SETOR) | `rwx` |
| 7 | Outros | `rwx` |

### Criação de Usuários

- A senha padrão para todos os usuários criados é definida na variável `SENHA_PADRAO` dentro do arquivo `config.conf`.
...
> **⚠️ AVISO DE SEGURANÇA**: A senha padrão é utilizada no nosso script é apenas para fins educacionais. Em um ambiente de produção real, utilize senhas fortes, chaves SSH ou integração com diretórios (LDAP/AD).
...
### Por que usamos redirecionamento 2>&1?
No Linux, os processos utilizam três descritores de arquivos padrão:
- **0 (stdin)**: Entrada de dados.
- **1 (stdout)**: Saída padrão (mensagens de sucesso).
- **2 (stderr)**: Saída de erro.

Ao utilizar `2>&1`, estamos instruindo o shell a redirecionar todas as mensagens de erro (canal 2) para o mesmo destino da saída padrão (canal 1). Isso garante que, ao salvar os logs em um arquivo com `>> log.txt`, tanto os sucessos quanto as falhas sejam registrados para auditoria futura.
